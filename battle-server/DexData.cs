...
namespace MonsterBattler.BattleServer
{
    ...
    public static class DexData
    {
        ...
        // Data dir: DEX_DIR env, else ./dex next to the binary, else the repo's StreamingAssets
        // (dev convenience). The Dockerfile copies the dex JSON into ./dex.
        static string DexDir([CallerFilePath] string thisFile = "")
        {
            var fromEnv = Environment.GetEnvironmentVariable("DEX_DIR");
            if (!string.IsNullOrEmpty(fromEnv)) return fromEnv;
            var beside = Path.Combine(AppContext.BaseDirectory, "dex");
            if (Directory.Exists(beside)) return beside;
            var srcDir = Path.GetDirectoryName(thisFile);
            return Path.GetFullPath(Path.Combine(srcDir, "..", "MonsterBattler", "Assets", "StreamingAssets", "dex"));
        }
