...
extern "C"
{
  ...
  int _coreHapticsCreateContinuousPlayer(float initialIntensity, float initialSharpness)
  {
    return [UnityCoreHaptics CreateContinuousPlayerWithInitialIntensity:initialIntensity initialSharpness:initialSharpness];
  }
  
  void _coreHapticsStartPlayer(int playerID)
  {
    [UnityCoreHaptics StartPlayerWithPlayerID:playerID];
  }
  
  void _coreHapticsStopPlayer(int playerID)
  {
    [UnityCoreHaptics StopPlayerWithPlayerID:playerID];
  }
  
  void _coreHapticsUpdatePlayerParameters(int playerID, float intensity, float sharpness)
  {
    [UnityCoreHaptics UpdatePlayerParametersWithPlayerID:playerID intensity:intensity sharpness:sharpness];
  }
  
  void _coreHapticsDestroyPlayer(int playerID)
  {
    [UnityCoreHaptics DestroyPlayerWithPlayerID:playerID];
  }
}
